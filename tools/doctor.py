...
def print_requirements_checklist() -> None:
    ...
    if ram_ok:
        ...
        if ram_gb > 0:
            out(f"  [✗] System RAM:         {ram_gb:.2f} GB (Failed - Min 1 GB required)")
        else:
            out(f"  [✗] System RAM:         Unknown (Failed)")
        
    if has_docker:
        out("  [✓] Docker Engine:      Installed (Passed)")
    else:
        out("  [ ] Docker Engine:      Not Found (Optional - only required for Docker container)")
...
def validate_profile(profile: str) -> None:
    ...
    try:
        ...
        if _doctor_mode:
            ...
            out("  1. Run './tools/run.sh setup' to automatically generate .env and docker-compose.yml.")
            out("  2. Ensure your Redis server is active and reachable via the REDIS_URL in .env.")
            out("  3. Start the worker container: 'docker compose up -d' (or python app/main.py).")
            out()
